...
User clicks Upgrade
      │
      ▼
SDK calls POST /api/public/auth/checkout-token
      │  (publicKey + tenantId)
      │
      ▼
Crovver returns a signed JWT
      │
      ▼
Browser redirects to → {ecomUrl}/pricing?token={jwt}
      │
      ▼
Ecom app validates token, fetches plans
User selects a plan
      │
      ▼
Ecom calls POST /api/public/checkout
      │
      ▼
Crovver creates Stripe checkout session
...
Crovver activates subscription
      │
      ▼
useSubscription() returns isActive: true ✅
```
